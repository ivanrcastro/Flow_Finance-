...
export default function BudgetChart({ currentMonthExpenses, lastMonthExpenses, totalSpent, remaining, budget }) {
  ...
  // Calculamos os totais usando o ID (inglês) mas preparamos o nome para exibição traduzida
  const categoryTotals = CATEGORIES.map(cat => {
    ...
    return { 
      id: cat, 
      name: t(`categories.${cat}`), // Nome traduzido para o gráfico
      value 
    };
      ...
      <div className="bg-white p-8 rounded-[32px] shadow-sm border border-slate-100 relative">
        <div className="h-56 w-full relative">
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              <Pie
                data={categoryTotals.length > 0 ? categoryTotals : [{ name: t('actions.no_data') || 'No Data', value: 1 }]}
